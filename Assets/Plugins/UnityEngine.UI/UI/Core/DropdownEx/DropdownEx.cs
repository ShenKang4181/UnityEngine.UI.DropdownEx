...
namespace UnityEngine.UI
{
    ...
    {        
        ...
        public void RefreshShownValue()
        {
            if (0 == options.Count)
            {
                if (m_CaptionText != null)
                    SetCaptionText(!string.IsNullOrEmpty(s_NoOptionData.text) ? s_NoOptionData.text : "");
            ...
            else
            {
                ...
                if (m_CaptionText != null)
                {
                    if (0 == itemCount)
                    {
                        SetCaptionText( !string.IsNullOrEmpty(NothingSelectedText) ? NothingSelectedText : "");
                    }
                    else if (1 == itemCount)
                    {
                        SetCaptionText( !string.IsNullOrEmpty(data.text) ? data.text : "");
                    }
                    else
                    {
                        // The only way bitcount can be > 1 is if AllowMultipleSelections is true
                        SetCaptionText( !string.IsNullOrEmpty(MultipleSelectedText) ? MultipleSelectedText : "");
                    }
                }
        ...
        public virtual void SetCaptionText( string text ) 
        {
            if ( m_CaptionText != null )
            {
